...
public class ClienteDAO {
	...
	/**
	 * public UsuarioVO auth(UsuarioVO usr) { WebClient webClient =
	 * WebClient.create(URL); UsuarioVO objUsuario = null;
	 * 
	...
	public ProductoVO nuevoProducto(ProductoVO p) {
		...
		Mono<ProductoVO> response = webClient.put().uri(URL + "/registrarProducto").body(Mono.just(p), ProductoVO.class)
				.retrieve().bodyToMono(ProductoVO.class);
	...
	public void borrarProductos() {
		try {
			URL url = new URL(URL + "/borrarProductos");// your url i.e fetch data from .
			HttpURLConnection conn = (HttpURLConnection) url.openConnection();
			conn.setRequestMethod("GET");
			conn.setRequestProperty("Accept", "application/json");
			if (conn.getResponseCode() != 200) {
